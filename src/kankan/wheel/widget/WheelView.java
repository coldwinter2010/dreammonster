...
//	private int wheelForeground = R.drawable.wheel_val;
	private int wheelBackground = R.color.background;
	private int wheelForeground = R.color.background;
	// Shadows drawables
	...
	private void initData(Context context) {
		scroller = new WheelScroller(getContext(), scrollingListener);//鏋勯�鍑芥暟閲屽厛鍒涘缓scroller
	}
	...
	WheelScroller.ScrollingListener scrollingListener = new WheelScroller.ScrollingListener() {
		...
		public void onJustify() {
			if (Math.abs(scrollingOffset) > WheelScroller.MIN_DELTA_FOR_SCROLLING) {
				scroller.scroll(scrollingOffset, 0);//姣忔鍙墽琛屼竴娆★紝鏈�悗婊戝姩鍥瀁瀵归綈鐨勪綅缃�
			}
		}
	};
...
//		}
	...
	private void doScroll(int delta) {
		...
		int count = scrollingOffset / itemHeight;//闇�绉诲姩鍑犱釜view鐨勪綅缃�
		int pos = currentItem - count;//闇�绉诲姩鍒扮殑view鐨刾os
		int itemCount = viewAdapter.getItemsCount();//鎬诲叡鐨刬tem鏁伴噺
		int fixPos = scrollingOffset % itemHeight;//
		if (Math.abs(fixPos) <= itemHeight / 2) {//濡傛灉灏戜簬itemHeight涓�崐灏辨妸fixPos娓�
			fixPos = 0;
		}
		if (isCyclic && itemCount > 0) {
			if (fixPos > 0) {//濡傛灉澶氫簡涓�鍗婏紝鍒檖os鍐嶇炕涓�釜
				pos--;
				count++;
			} else if (fixPos < 0) {
				pos++;
	...
	private boolean addViewItem(int index, boolean first) {
		TextView view=(TextView)getItemView(index);
		if (view != null) {
			if (first) {
	...
	private void testFunc(){
		TextView view = null;
		int currentPos=currentItem-firstItem;
		for(int i=0;i<itemsLayout.getChildCount();i++){
			view=(TextView) itemsLayout.getChildAt(i);
			if(i==currentPos-1||i==currentPos+1){
				view.setTextSize(28);
				view.setPadding(0, 2, 0, 2);
				
			}else if(i==currentPos-2||i==currentPos+2){
				view.setTextSize(24);
